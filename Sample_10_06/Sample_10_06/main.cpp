...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	// gaussianBlur[1]～[3]は前のガウシアンブラーの結果にガウシアンブラーをかける
	for (int i = 1; i < 4; i++)
	{
		gaussianBlur[i].Init(&gaussianBlur[i - 1].GetBokeTexture());
	}
	...
	for (int i = 0; i < 4; i++)
	{
		finalSpriteInitData.m_textures[i] = &gaussianBlur[i].GetBokeTexture();
	}
	...
	// カラーバッファーのフォーマットは例によって32ビット浮動小数点
	finalSpriteInitData.m_colorBufferFormat[0] = DXGI_FORMAT_R32G32B32A32_FLOAT;

	// 初期化情報をもとにスプライトを初期化する
	Sprite finalSprite;
