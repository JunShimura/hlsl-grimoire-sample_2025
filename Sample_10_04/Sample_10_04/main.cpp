...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	mainRenderTarget.Create(
		FRAME_BUFFER_W,   // レンダリングターゲットの幅
		FRAME_BUFFER_H, // レンダリングターゲットの高さ
		1,
		1,
	...
	const int NUM_WEIGHTS = 8; // 重みテーブルの要素数

	// テーブルのサイズの配列を作る
	float weights[NUM_WEIGHTS];

	// ガウス関数を用いて重みテーブルを計算する
	CalcWeightsTableFromGaussian(
		weights,    // 重みの格納先
		NUM_WEIGHTS,    // 重みテーブルの要素数
		8.0f);      // 分散具合(この値が大きいほど強くぼかされる)

	// step-3 横ブラー用のレンダリングターゲットを作成
	RenderTarget xBlurRenderTarget;
	xBlurRenderTarget.Create(
		FRAME_BUFFER_W / 2,   // レンダリングターゲットの幅は半分
	...
	// 横ブラー用スプライトの初期化情報を設定する
	SpriteInitData xBlurSpriteInitData;
	xBlurSpriteInitData.m_fxFilePath = "Assets/shader/samplePostEffect.fx";
	xBlurSpriteInitData.m_vsEntryPointFunc = "VSXBlur";   // 頂点シェーダー関数名
	xBlurSpriteInitData.m_psEntryPoinFunc = "PSBlur";   // 頂点シェーダー関数名

	// スプライトの解像度はXbulurRenderTargetと同じにする
	xBlurSpriteInitData.m_width = FRAME_BUFFER_W / 2;    // 幅はレンダリングターゲットと同じ
	xBlurSpriteInitData.m_height = FRAME_BUFFER_H;       // 高さはレンダリングターゲットと同じ

	// 【注目】テクスチャはmainRenderTargetのカラーバッファー
	xBlurSpriteInitData.m_textures[0] = &mainRenderTarget.GetRenderTargetTexture();

	// 【注目】ユーザー拡張変数の定数バッファーに重みテーブルを設定する
	...
	yBlurRenderTarget.Create(
		FRAME_BUFFER_W / 2,       // xBlurRenderTargetと同じ幅
		FRAME_BUFFER_H / 2,     // 高さは半分
		1,
		1,
		DXGI_FORMAT_R8G8B8A8_UNORM, // レンダリングターゲットのフォーマット
		DXGI_FORMAT_D32_FLOAT // 深度ステンシルのフォーマット
	...
	yBlurSpriteInitData.m_vsEntryPointFunc = "VSYBlur";   // 頂点シェーダー関数名
	yBlurSpriteInitData.m_psEntryPoinFunc = "PSBlur";   // 頂点シェーダー関数名

	// スプライトの解像度はyBlurRenderTargetと同じにする
	yBlurSpriteInitData.m_width = FRAME_BUFFER_W / 2;    // 幅はレンダリングターゲットと同じ
	yBlurSpriteInitData.m_height = FRAME_BUFFER_H / 2;   // 高さはレンダリングターゲットと同じ

	// テクスチャはxBlurRenderTargetのカラーバッファー
	yBlurSpriteInitData.m_textures[0] = &xBlurRenderTarget.GetRenderTargetTexture();

	// ユーザー拡張変数の定数バッファーに重みテーブルを設定する
	...
	// ボケ画像をそのまま張り付けるだけなので、通常のスプライトシェーダーを使う
