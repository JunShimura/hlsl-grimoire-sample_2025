...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
    ...
    while (DispatchWindowMessage())
    {
        ...
        // 注意: Dispatch の引数は "スレッドグループ数" を指定する。
        //       本サンプルのコンピュートシェーダ側では [numthreads(4, 4, 1)] を使っているため
        //       1 グループが 4x4 ピクセルを処理する。
        //       したがってピクセル単位のテクスチャ解像度を 4 で割った値をグループ数として渡している。
        //       （サイズが4で割り切れない場合は切り上げが必要だが、本例では割り切れる前提で簡略化している）
