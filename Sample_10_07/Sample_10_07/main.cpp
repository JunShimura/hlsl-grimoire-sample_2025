...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	// 使用するテクスチャは2枚
	combineBokeImageSpriteInitData.m_textures[0] = &blur.GetBokeTexture();          // ボケ画像
	combineBokeImageSpriteInitData.m_textures[1] = &depthRenderTarget.GetRenderTargetTexture(); // 深度テクスチャ
	...
	// 距離を利用してボケ画像をαブレンディングするので、半透過合成モードにする
	...
	while (DispatchWindowMessage())
	{
		...
		renderContext.WaitUntilToPossibleSetRenderTargets(_countof(rts), rts);

		// レンダリングターゲットを設定する
		renderContext.SetRenderTargets(_countof(rts), rts);

		// レンダリングターゲットをクリア
		renderContext.ClearRenderTargetViews(_countof(rts), rts);

		// モデルをドロー
		model.Draw(renderContext);

		// レンダリングターゲットへの書き込み待ち
		renderContext.WaitUntilFinishDrawingToRenderTargets(_countof(rts), rts);
