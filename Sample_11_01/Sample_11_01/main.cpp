...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	shadowMap.Create(
		1024, // 【注目】シャドウマップの解像度は1024x1024にする
		1024,
		1,
		1,
	...
	// カメラの位置を設定。これはライトの位置
	lightCamera.SetPosition({ 0.0f, 600.0f, 0.0f });

	// カメラの注視点を設定。これがライトが照らしている場所
	lightCamera.SetTarget(0.0f, 0.0f, 0.0f);

	// 【注目】上方向を設定。今回はカメラが真下を向いているのでX方向を上にしている
	...
	ModelInitData teapodShadowModelInitData;

	// 【注目】シャドウマップ描画用のシェーダーを指定する
	teapodShadowModelInitData.m_fxFilePath = "Assets/shader/sampleDrawShadowMap.fx";
	teapodShadowModelInitData.m_tkmFilePath = "Assets/modelData/teapot.tkm";
	Model teapotShadowModel;
	teapotShadowModel.Init(teapodShadowModelInitData);
