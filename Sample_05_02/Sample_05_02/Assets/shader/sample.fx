...
float4 PSMain(SPSIn psIn) : SV_Target0
{
    ...
    // dot()を利用して、ライトの方向とスポットライトの方向の内積を求める
	float angle = dot(ligDir, spDirection);
    
    // dot()で求めた値をacos()関数に渡して、ラジアン角に変換する
	angle = abs(acos(angle));

    // step-12 角度による影響率を求める
    // 角度に比例して小さくなっていく影響率を計算する
	affect = 1.0f - 1.0 / spAngle * angle;

    // 影響率がマイナスにならないように補正をかける
	affect = max(0.0f, affect);
    
    // 拡散反射光と鏡面反射光に角度による影響率を乗算して影響を弱める
	affect = pow(affect, 0.5f); // 影響の仕方を指数関数的にする
