...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	while (DispatchWindowMessage())
	{
		...
		for (int areaNo = 0; areaNo < NUM_SHADOW_MAP; areaNo++)
		{
			...
			for (auto& v : vertex)
			{
				lvpMatrix.Apply(v);  // 頂点に変換行列を適用（座標変換）
				vMax.Max(v);          // vMaxの各成分をvと比較して大きい方を保持
				vMin.Min(v);          // vMinの各成分をvと比較して小さい方を保持
			}


			// step-9 クロップ行列を求める
			float xScale = 2.0f / (vMax.x - vMin.x);  // X軸のスケール
			float yScale = 2.0f / (vMax.y - vMin.y);  // Y軸のスケール
